...
from reachy_mini_conversation_app.smart_turn.vad import SileroVAD
...
# ── SmartTurn / VAD configuration ────────────────────────────────────────
_SMART_TURN_SR = 16_000  # Silero VAD + SmartTurn expect 16 kHz mono
_SILERO_CHUNK = SileroVAD.CHUNK  # 512 samples @ 16 kHz
_CHUNK_MS = (_SILERO_CHUNK / _SMART_TURN_SR) * 1000.0
...
class OpenaiRealtimeHandler(AsyncStreamHandler):
    ...
    def __init__(self, deps: ToolDependencies, gradio_mode: bool = False, instance_path: Optional[str] = None):
        ...
        # ── SmartTurn / VAD state ─────────────────────────────────────────
        self._vad = SileroVAD()
        self._vad_residual: np.ndarray = np.empty(0, dtype=np.float32)
    ...
    async def _run_realtime_session(self) -> None:
        ...
        async with self.client.realtime.connect(model=config.MODEL_NAME) as conn:
            ...
            # Reset SmartTurn VAD state for this session
    ...
    async def receive(self, frame: Tuple[int, NDArray[np.int16]]) -> None:
        ...
        while len(self._vad_residual) >= _SILERO_CHUNK:
            vad_chunk = self._vad_residual[:_SILERO_CHUNK]
            self._vad_residual = self._vad_residual[_SILERO_CHUNK:]

            is_speech = self._vad.prob(vad_chunk) > VAD_THRESHOLD

            if not self._is_speaking:
                self._pre_buffer.append(vad_chunk)
                ...
                if is_speech:
                    ...
                    if self._response_active:
                        try:
                            ...
                            logger.debug("Could not cancel response: %s", e)
    ...
    def _reset_vad_state(self) -> None:
        """Reset all SmartTurn / VAD state for the next utterance."""
        self._is_speaking = False
        self._user_turn_active = False
        self._trailing_silence_chunks = 0
        self._since_trigger_chunks = 0
        self._segment_chunks.clear()
        self._pre_buffer.clear()

    @staticmethod
    def _resample_to_16k(samples: NDArray[np.int16], input_sr: int) -> np.ndarray:
        """Resample int16 audio from *input_sr* → 16 kHz float32 in [-1, 1]."""
        float_samples = samples.astype(np.float32) / 32768.0
        if float_samples.ndim > 1:
            float_samples = float_samples[:, 0]
        if input_sr != _SMART_TURN_SR:
            n_out = int(len(float_samples) * _SMART_TURN_SR / input_sr)
            float_samples = resample(float_samples, n_out)
